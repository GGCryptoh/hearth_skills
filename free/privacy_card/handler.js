...
//
// Six verbs: mint / list / freeze / unfreeze / set_limit / transactions.
//
...
//
// The card details returned by mint (PAN + CVV + exp) are the agent's
// responsibility to use safely — they typically go straight into a
// merchant checkout form and never get written to disk.
...
export async function run(ctx, args) {
  ...
  switch (verb) {
    ...
    default:
      throw new Error(
        `unknown verb: ${verb}. Supported: mint, list, freeze, unfreeze, set_limit, transactions.`,
      );
  }
}
...
async function list(args, apiKey) {
  ...
  const res = await fetch(`${API_BASE}/card?page=${page}&page_size=50`, {
    headers: authHeaders(apiKey),
  });
  if (!res.ok) return errorBody(res);
...
async function transactions(args, apiKey) {
  ...
  if (args.begin_iso) params.set('begin', String(args.begin_iso));
  params.set('page_size', '50');
  const res = await fetch(`${API_BASE}/transaction?${params}`, {
    headers: authHeaders(apiKey),
  });
  if (!res.ok) return errorBody(res);
  const json = await res.json();
