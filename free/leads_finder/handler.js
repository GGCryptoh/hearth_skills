// B2B Leads Finder — find leads from a search query.
//
// Uses Apify's code_crafter/leads-finder actor.
//
// Args:
//   query  string  required — free-form search query
//   limit  number  optional — max leads (default 10, max 100)
//
// Returns: { ok, items, item_count, run_id, summary }

const API_BASE = 'https://api.apify.com/v2';
const ACTOR_SLUG = 'code_crafter~leads-finder';
const SUPERVISOR_BASE = 'http://127.0.0.1:3417';
const SKILL_ID = 'leads_finder';
...
export async function run(ctx, args) {
  ...
  if (!query) {
    throw new Error(
      "query is required (e.g. 'CTOs at SaaS companies in San Francisco')",
    );
  }
  ...
  return {
    ...
    run_id: runId,
    summary: `Leads finder "${query}": ${arr.length} lead${arr.length === 1 ? '' : 's'}`,
  };
}
